...
if not payment_id and not reference:
    logger.error(
        "DOMPETX WEBHOOK | PAYMENT ID AND REFERENCE MISSING"
    )
    return {
        "success": False,
...
#
# Kita cek ulang transaksi langsung ke DompetX.
#
# Jika ada payment_id:
#     /v1/payments/check-status/{paymentId}
#
# Jika hanya reference:
#     /v1/payments/check-status?reference={reference}
#
# =====================================================
verified = None
try:
...
if verified_status not in SUCCESS_STATUS:
    logger.info(
        "DOMPETX PAYMENT NOT SUCCESS | status=%s",
        verified_status
    )
    return {
        "success": True,
...
async with pool.acquire() as conn:
    async with conn.transaction():
        ...
        if locked_purchase["status"] == "paid":
            logger.info(
                "FILE PAYMENT ALREADY PAID AFTER LOCK | %s",
                invoice_id
            )
            return {
                "success": True,
...
# =====================================================
try:
    qr_chat_id = purchase["qr_chat_id"]
    qr_message_id = purchase["qr_message_id"]
...
# bisa menghasilkan datetime naive.
#
# Normalisasi supaya aman dibandingkan dengan
